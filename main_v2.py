...
def initialize_world():
    ...
    print(f"world rank: {world_rank}, local_rank: {local_rank}, world_size: {world_size}, master addr: {os.environ['MASTER_ADDR']}, torch device: {torch.cuda.current_device()}, visible devices: {os.environ['CUDA_VISIBLE_DEVICES']}, device_count: {torch.cuda.device_count()}", flush=True)
    ...
    #print("MA =", os.environ['MASTER_ADDR'])
    #master_addr = os.environ['MASTER_ADDR']
    #os.environ['MASTER_PORT'] = master_port
    #os.environ['WORLD_SIZE'] = world_size
    #os.environ['RANK'] = world_rank
    #os.environ['LOCAL_RANK'] = str(int(world_rank) % 6)
    #rank = int(world_rank)#torch.distributed.get_rank()
    #world_size = int(world_size)#torch.distributed.get_world_size()
    #local_rank = int(os.environ['LOCAL_RANK'])
    #print("World view: ", rank, world_size, master_addr)
...
def setup():
    ...
    if dist.is_initialized():
       ...
       return world_size, world_rank, get_local_rank()
...
def train_gnet(args, config):
    ...
    #world_size, world_rank, local_rank = setup()
    ...
    for cluster_idx in range(num_clusters):
        ...
        for epoch in range(config['num_epoch']):
            ...
            for images, _, _ in logger.log_every(train_loader, 50, header=header):
                ...
                anchors = torch.stack([cluster['centers'][i] for i in target_labels], dim=0).to(local_rank)
